"""Run the frozen HRM-Text P1 gold-evidence development smoke."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the pinned frozen HRM-Text train-only interface smoke."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    with dataset_manifest_path.open("r", encoding="utf-8") as stream:
    ...
    if len(examples) != int(dataset_config["selection"]["size"]):
        raise ValueError("development-pool size mismatch")
    ...
    if len(demonstrations) != 2 or len(targets) != evaluated_count:
        raise ValueError("D009 requires exactly 2 demonstrations and 16 targets")

    output_dir = _resolve(root, runtime["output_dir"])
    ...
    try:
        ...
        write_json_atomic(
            run_dir / "statistics.json",
            {
                ...
                "reason": (
                    "Train-only n=16 interface smoke; no model comparison or "
                    "confirmatory claim."
                ),
            },
        )
    ...
    *,
    ...
    with (run_dir / "predictions.jsonl").open(
        ...
        for target in targets:
            ...
            extraction = extract_short_answer(raw_output)
    ...
    metrics = {
        ...
        "claim_eligibility": "none; P1 train-only interface smoke",
    }
    environment = {
        "python": sys.version,

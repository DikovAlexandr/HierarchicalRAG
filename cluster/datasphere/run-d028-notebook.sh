...
CONFIG="experiments/configs/p2-qwen3-embedding-fullwiki-calibration-v1.yaml"
EXPERIMENT_ID="p2-qwen3-embedding-fullwiki-calibration-v1"
...
PACKAGE_DIR=""
for candidate in "${ROOT}/.d023-python-packages" "${ROOT}/.d017-python-packages" "${ROOT}/.d028-python-packages"; do
...
if [[ -z "${PACKAGE_DIR}" ]]; then
  PACKAGE_DIR="${ROOT}/.d028-python-packages"
  if [[ -e "${PACKAGE_DIR}" ]]; then
    echo "Incomplete D028 package directory already exists: ${PACKAGE_DIR}" >&2
    exit 2
  fi
...
set +e
(
  echo "stage=dense_calibration_start experiment_id=${EXPERIMENT_ID}"
  PYTHONPATH="${PACKAGE_DIR}:src" "${BOOTSTRAP_PYTHON}" \
    -m hierarchical_rag.run_dense_calibration --config "${CONFIG}"
) 2>&1 | tee "${LOG_FILE}"
STATUS=${PIPESTATUS[0]}
set -e
